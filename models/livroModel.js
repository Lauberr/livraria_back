...
module.exports = {
  ...
  criar: async (dados) => {
    const {
      ...
      autores = [], editora, categoria = [], data_publicacao
    } = dados;
    ...
    for (const nome of autores) {
      let autor = await db.query('SELECT id_autor FROM autores WHERE nome_autor = $1', [nome]);
      if (autor.rows.length === 0) {
        autor = await db.query('INSERT INTO autores (nome_autor) VALUES ($1) RETURNING id_autor', [nome]);
      }
      await db.query('INSERT INTO autor_livro (id_livro, id_autor) VALUES ($1, $2)', [id_livro, autor.rows[0].id_autor]);
    }

    if (editora) {
      let ed = await db.query('SELECT id_editora FROM editora WHERE nome_editora = $1', [editora]);
      if (ed.rows.length === 0) {
        ed = await db.query('INSERT INTO editora (nome_editora, data_publicacao) VALUES ($1, $2) RETURNING id_editora', [editora, data_publicacao || null]);
      }
      await db.query('INSERT INTO editora_livro (id_livro, id_editora) VALUES ($1, $2)', [id_livro, ed.rows[0].id_editora]);
    }

    for (const nome of categoria) {
      let cat = await db.query('SELECT id_cat FROM categoria WHERE nome_cat = $1', [nome]);
      if (cat.rows.length === 0) {
        cat = await db.query('INSERT INTO categoria (nome_cat) VALUES ($1) RETURNING id_cat', [nome]);
      }
      await db.query('INSERT INTO categoria_livro (id_livro, id_cat) VALUES ($1, $2)', [id_livro, cat.rows[0].id_cat]);
    }
